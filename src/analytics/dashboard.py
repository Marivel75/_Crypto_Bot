...
class Dashboard:
    ...
    def fetch_data(self, symbol: str = "BTC/USDT", limit: int = 100) -> None:
        ...
        try:
            ...
            self.btc_data = self.db_inspector.get_ohlcv_data(symbol=symbol, limit=limit)

            if self.btc_data is not None and not self.btc_data.empty:
                display(self.btc_data.head())
    ...
    def plot_ohlcv(self, title: str = "BTC/USDT - Données OHLCV") -> None:
        ...
        if self.btc_data is not None and not self.btc_data.empty:
            try:
                ...
                self.plot_manager.plot_ohlcv(self.btc_data, title=title)
            except Exception as e:
    ...
    def plot_sma(
        ...
        if self.btc_data is not None and not self.btc_data.empty:
            try:
                ...
                sma_serie = calculate_sma(self.btc_data)
                self.plot_manager.plot_with_sma(
                    self.btc_data, sma_serie, window=window, title=title
                )
            except Exception as e:
                self.logger.error(
