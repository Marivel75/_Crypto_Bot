...
from src.etl.ohlcv_pipeline.pipeline_ohlcv import (
...
class TestETLPipelineExtractTransformBatch:
    ...
    def test_extract_transform_batch_success(self):
        ...
        assert len(results) == 2
        assert all(
            isinstance(r, pd.DataFrame) for r in results.values() if r is not None
        )
...
class TestETLPipelineSummary:
    ...
    def test_get_summary_success(self):
        ...
        results = {
            "BTC/USDT": self._create_success_result(
                "BTC/USDT", "1h", 100, 95, 95, 1.0, 2.0, 3.0
            ),
            "ETH/USDT": self._create_success_result(
                "ETH/USDT", "1h", 100, 98, 98, 1.5, 2.5, 4.0
            ),
        }
    ...
    def test_get_summary_partial_failure(self):
        ...
        results = {
            ...
            "ETH/USDT": self._create_failed_result(
                "ETH/USDT", "1h", "Transformation error"
            ),
        }
