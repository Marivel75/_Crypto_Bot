...
from src.shared.models.orm import OHLCVOrm, UserOrm
...
class TestPortfolioCRUD:
    ...
    async def test_add_portfolio_entry_symbol_uppercase(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        entry = await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "btc", 1.0, 50000.0)
        assert entry.symbol == "BTC"
    ...
    async def test_get_portfolio_after_add(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "ETH", 2.0, 2000.0)
        entries = await user_data_service.get_portfolio(db_session, str(test_user.id))
    ...
    async def test_update_portfolio_entry_quantity(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        entry = await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "SOL", 10.0, 100.0)
    ...
    async def test_update_portfolio_entry_price(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        """Update entry price."""
        entry = await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "ADA", 100.0, 0.50)
        updated = await user_data_service.update_portfolio_entry(
    ...
    async def test_update_portfolio_entry_wrong_owner(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        entry = await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "DOGE", 1000.0, 0.10)
        with pytest.raises(AuthorizationError):
            await user_data_service.update_portfolio_entry(db_session, "other-user-id", str(entry.id), quantity=2000.0)

    @pytest.mark.asyncio
    async def test_delete_portfolio_entry(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        """Delete removes the entry."""
        entry = await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "XRP", 500.0, 1.00)
    ...
    async def test_delete_portfolio_entry_not_found(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        with pytest.raises(NotFoundError):
            await user_data_service.delete_portfolio_entry(db_session, str(test_user.id), "nonexistent-id")
    ...
    async def test_delete_portfolio_entry_wrong_owner(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        entry = await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "USDT", 1000.0, 1.00)
        with pytest.raises(AuthorizationError):
            await user_data_service.delete_portfolio_entry(db_session, "other-user-id", str(entry.id))
...
class TestWatchlistCRUD:
    ...
    async def test_add_watchlist_symbol(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        entry = await user_data_service.add_watchlist_symbol(db_session, str(test_user.id), "BTC")
        assert entry.symbol == "BTC"

    @pytest.mark.asyncio
    async def test_add_watchlist_symbol_uppercase(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        """Symbol is automatically uppercased."""
        entry = await user_data_service.add_watchlist_symbol(db_session, str(test_user.id), "eth")
    ...
    async def test_add_watchlist_duplicate_raises(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        await user_data_service.add_watchlist_symbol(db_session, str(test_user.id), "SOL")
    ...
    async def test_remove_watchlist_symbol(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        await user_data_service.add_watchlist_symbol(db_session, str(test_user.id), "ADA")
        await user_data_service.remove_watchlist_symbol(db_session, str(test_user.id), "ADA")
        entries = await user_data_service.get_watchlist(db_session, str(test_user.id))
        assert len(entries) == 0
    ...
    async def test_remove_watchlist_not_found(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        with pytest.raises(NotFoundError):
            await user_data_service.remove_watchlist_symbol(db_session, str(test_user.id), "NONEXISTENT")
...
class TestPortfolioSummary:
    ...
    async def test_portfolio_summary_single_entry(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        await user_data_service.add_portfolio_entry(db_session, str(test_user.id), "BTC", 1.0, 50000.0)
        summary = await user_data_service.get_portfolio_summary(db_session, str(test_user.id))
...
class TestWatchlistPrices:
    ...
    @pytest.mark.asyncio
    async def test_watchlist_prices_with_data(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        assert prices[0]["timestamp"].replace(tzinfo=timezone.utc) == now

    @pytest.mark.asyncio
    async def test_watchlist_prices_latest_only(self, db_session: AsyncSession, test_user: UserOrm) -> None:
        ...
        assert prices[0]["timestamp"].replace(tzinfo=timezone.utc) == new_time
