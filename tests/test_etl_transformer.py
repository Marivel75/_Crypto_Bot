...
from src.etl.ohlcv_pipeline.transformer import (
    OHLCVTransformer,
    TransformationError,
)
...
class TestOHLCVTransformerTransform:
    ...
    def test_transform_success(self):
        ...
        mock_validator = MagicMock(spec=DataValidator0HCLV)
        mock_validator.validate_ohlcv_values.return_value = (
            True,
            {"valid_rows": 2, "total_rows": 2, "errors": [], "warnings": []},
        )
    ...
    def test_transform_data_enrichment(self):
        ...
        mock_validator.validate_ohlcv_values.return_value = (
            True,
            {"valid_rows": 1, "total_rows": 1, "errors": [], "warnings": []},
        )
        ...
        assert (
            abs(result["price_change_pct"].iloc[0] - 3.1578947) < 0.000001
        )  # (3/95)*100
...
class TestOHLCVTransformerBatch:
    ...
    def test_transform_batch_partial_failure(self):
        ...
        batch_data = {
            "BTC/USDT": [[1768294800000, 90000.0, 90100.0, 89900.0, 90050.0, 123.45]],
            "ETH/USDT": [
                [1768294800000, -3000.0, 3010.0, 2990.0, 3005.0, 456.78]
            ],  # Prix négatif
        }
