            ...
            def mock_to_sql(*args, **kwargs):
                ...
                if "method" in kwargs and kwargs["method"] is not None:
                    return kwargs["method"](
                        *args[:-1], **{k: v for k, v in kwargs.items() if k != "method"}
                    )
...
class TestOHLCVLoaderBatchInsert:
    ...
    def test_batch_insert_with_integrity_error(self):
        ...
        with patch("pandas.DataFrame.to_sql") as mock_to_sql:
            ...
            mock_to_sql.side_effect = [
                None,
                IntegrityError("mock", "mock", "Duplicate"),
            ]
...
class TestOHLCVLoaderBatchOperations:
    ...
    def test_load_batch_with_none_values(self):
        ...
        loader = OHLCVLoader(mock_engine)
        batch_data = {"BTC/USDT": pd.DataFrame({"col1": [1]}), "ETH/USDT": None}
...
class TestOHLCVLoaderTableOperations:
    ...
    def test_get_table_info(self):
        ...
        mock_engine.connect.return_value.execution_options.return_value.connection = (
            mock_connection
        )
